...
class step5 {
    ...
    public static function render(array $ctx): void {
        ...
        echo html_writer::tag('p', 'Revisa y selecciona los mejores ítems para tu evaluación', ['class' => 'rubricai-stitle']);
        ...
        if (empty($inst_content)) {
            ...
            echo \local_rubricai\step_renderer::render_preview_button(5);
            ...
            if (!$data || !is_array($data)) {
                ...
                foreach (($data['items'] ?? []) as $index => $item) {
                    ...
                    echo html_writer::tag('span', $item['difficulty'] ?? 'N/A', ['class' => 'rubricai-tag', 'style' => 'background:#fff3e0; color:#e65100; font-size:10px;']);
                    if (!empty($item['points'])) {
                        echo html_writer::tag('span', $item['points'] . ' pts', ['class' => 'rubricai-tag', 'style' => 'background:#e6f4ea; color:#1e8e3e; font-size:10px;']);
                    }
                    echo html_writer::end_tag('div');
                    echo html_writer::end_tag('div');
                ...
                echo html_writer::end_tag('div'); // rubricai-inner div wrapper (items container)
